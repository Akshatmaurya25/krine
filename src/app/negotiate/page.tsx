...
export default function NegotiatePage() {
  ...
  useEffect(() => {
    if (isSuccess && hash) {
      // Read the negotiation ID from the transaction receipt
      // For now, we'll fetch the latest negotiation count and navigate
      // In production, you'd parse the event from the receipt
      const fetchAndRedirect = async () => {
        try {
          // Wait a bit for the transaction to be indexed
          await new Promise(resolve => setTimeout(resolve, 2000));

          // Get the negotiation count (the new negotiation ID would be count - 1)
          const response = await fetch('/api/negotiation-count'); // You could implement this or use a contract read

          // For now, just navigate to inbox where user can see their negotiations
          router.push('/inbox');
        } catch (err) {
          console.error('Error fetching negotiation ID:', err);
          router.push('/inbox');
        }
      };
